...
class eGela:
    ...
    def check_credentials(self, username, password, event=None):
        ...
        if respuesta.status_code == 200:
            ...
            cuerpo_codificado = urllib.parse.urlencode(cuerpo)
            cookie_valor = respuesta.cookies.get('MoodleSessionegela')
            cabeceras = {'Host': "egela.ehu.eus",
                         'Content-Type': "application/x-www-form-urlencoded",
                         'Content-Length': str(len(cuerpo_codificado)),
            ...
            respuesta = requests.request(metodo, uri1, headers=cabeceras, data=cuerpo_codificado, allow_redirects=False)
    ...
    def get_pdf(self, selection):
        ...
        # Usar Session para que la cookie se envíe en todas las redirecciones automáticamente
        session = requests.Session()
        session.headers.update(cabeceras)
        session.cookies.set('MoodleSessionegela', self._cookie)

        # Añadir redirect=1 para forzar descarga directa
        if '?' in pdf_link:
            url_descarga = pdf_link + "&redirect=1"
        else:
            url_descarga = pdf_link + "?redirect=1"

        respuesta = session.get(url_descarga, allow_redirects=True)

        # Si aún devuelve HTML, buscar el enlace real pluginfile.php
        content_type = respuesta.headers.get('Content-Type', '')
        if 'text/html' in content_type:
            soup = BeautifulSoup(respuesta.content, 'html.parser')
            link_real = soup.find('a', href=re.compile(r'pluginfile\.php'))
            if link_real:
                print(f"\t  Siguiendo enlace pluginfile: {link_real['href'][:60]}...")
                respuesta = session.get(link_real['href'], allow_redirects=True)

        pdf_content = respuesta.content
        print(f"\tDescargado: {pdf_name} ({len(pdf_content)} bytes, tipo: {respuesta.headers.get('Content-Type')})")
        print(f"\tPrimeros bytes: {pdf_content[:10]}")  # Debe empezar por b'%PDF'
